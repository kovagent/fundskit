...
//!
//! ```text
//! $FUNDSKIT_CACHE_DIR/          (default: XDG cache / fundskit)
//! ├── fund13f-2024Q1.parquet        ← cached body
//! ├── fund13f-2024Q1.parquet.etag
//! ├── fund13f-2023Q4.parquet
//! └── fund13f-2023Q4.parquet.etag
//! ```
//!
//! # Fetch flow (per call)
//!
...
impl CachedFetcher {
    pub fn new(http: reqwest::Client, base_url: String, cache_dir: PathBuf) -> Self {
        let mirror_url = Some(
            std::env::var("FUNDSKIT_MIRROR_URL").unwrap_or_else(|_| DEFAULT_MIRROR_URL.to_string()),
        );
        Self {
            http,
            base_url,
    ...
    /// Fetch a parquet file by logical key (e.g. `"fund13f-2024Q1"`).
    ///
...
///
/// Priority:
/// 1. `$FUNDSKIT_CACHE_DIR` env var.
/// 2. XDG/platform cache dir for the `fundskit` application.
/// 3. Fallback: `~/.cache/fundskit`.
pub(crate) fn default_cache_dir() -> PathBuf {
    if let Ok(dir) = std::env::var("FUNDSKIT_CACHE_DIR") {
        return PathBuf::from(dir);
    }
    if let Some(proj) = directories::ProjectDirs::from("", "", "fundskit") {
...
fn dirs_fallback() -> PathBuf {
    #[cfg(target_os = "windows")]
    {
        std::env::var("LOCALAPPDATA")
            .map(|d| PathBuf::from(d).join("fundskit").join("cache"))
            .unwrap_or_else(|_| PathBuf::from("fundskit-cache"))
    }
    #[cfg(not(target_os = "windows"))]
    {
        std::env::var("HOME")
            .map(|h| PathBuf::from(h).join(".cache").join("fundskit"))
            .unwrap_or_else(|_| PathBuf::from(".fundskit-cache"))
    }
}
...
mod tests {
    ...
    async fn etag_304_returns_cached() {
        ...
        let body = b"fund13f-parquet-bytes";
        let etag_value = "\"abc123\"";

        Mock::given(method("GET"))
            .and(path("/fund13f-2024Q1.parquet"))
            .respond_with(
        ...
        let first = fetcher.fetch("fund13f-2024Q1").await.unwrap();
        assert_eq!(first.as_ref(), body);
        let second = fetcher.fetch("fund13f-2024Q1").await.unwrap();
    ...
    async fn stale_cache_fallback() {
        ...
        let server = MockServer::start().await;
        let stale_body = b"stale-fund13f-data";
        ...
        let http = reqwest::Client::builder()
            ...
            .unwrap();
        let cache_dir = tempfile::TempDir::new().unwrap();
        let cache_file = cache_dir.path().join("fund13f-2024Q1.parquet");
        tokio::fs::write(&cache_file, stale_body).await.unwrap();

        let mut fetcher = CachedFetcher::new(http, server.uri(), cache_dir.path().to_path_buf());
        fetcher.set_mirror_url(None);

        let result = fetcher.fetch("fund13f-2024Q1").await.unwrap();
    ...
    async fn corrupt_stale_cache_rejected_when_manifest_present() {
        ...
        let manifest_json = format!(r#"{{"fund13f-2024Q1.parquet":"sha256:{correct_digest}"}}"#);
        ...
        let http = reqwest::Client::builder()
            ...
            .unwrap();
        let cache_dir = tempfile::TempDir::new().unwrap();
        tokio::fs::write(cache_dir.path().join("fund13f-2024Q1.parquet"), b"CORRUPT")
            .await
            .unwrap();
